...
| `repositories-actions.spec.ts`        | the write side of the Repositories page (#2399): Create repository's dry run as the manager renders it, Create → the pull request opened as the person and the new repository's set-up steps live (the commit and the record stubbed at the browser; every other call reaches the manager), a row's Archive dialog naming its effect and the team's review, the catalog's Create… landing on the form (needs `app.routes.bindings: {catalog.createComponent: repositories.create}` in the lab's app-config). Skipped unless `AGENTLAB_REPO_MANAGER=1`              |
