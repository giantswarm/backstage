...
| `repositories-actions.spec.ts`        | the write side of the Repositories page (#2399, #2427, #2428): Create repository's intro (create → scaffold → pull request as the person) and its dry run as the manager renders it — a Go service with the CircleCI generator on, a configuration repository refused until the refusal's fix (one click) turns _Generate CircleCI config_ off and Review passes; Create → the repository, its scaffold commit and the pull request named as the person, the new repository's set-up steps live (the commit and the record stubbed at the browser; every other call reaches the manager); a row's Archive dialog naming its effect and the team's review; the catalog's Create… landing on the form (needs `app.routes.bindings: {catalog.createComponent: repositories.create}` in the lab's app-config). Skipped unless `AGENTLAB_REPO_MANAGER=1` |
