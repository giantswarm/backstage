...
export class KubernetesClientFactory {
  ...
  private initializeClusters() {
    try {
      ...
      for (const locatorConfig of clusterConfigs) {
        ...
        if (type === 'config') {
          ...
          for (const clusterConfig of clusters) {
            ...
            switch (authProvider) {
              case 'serviceAccount': {
                const serviceAccountToken = clusterConfig.getString(
                  'serviceAccountToken',
                );
                kubeConfig.addUser({
                  name: name,
              ...
              case 'oidc':
                this.logger.warn(
                  `OIDC auth provider: ${authProvider} for cluster ${name}`,
                );
                kubeConfig.addUser({
                  name: name,
                  authProvider: 'oidc',
              ...
              default:
                this.logger.warn(
                  `Unsupported auth provider: ${authProvider} for cluster ${name}, falling back to default`,
                );
            ...
            this.logger.info(
              `Added Kubernetes cluster "${name}" from app-config`,
            );
          }
        } else {
          this.logger.info(
            `Cluster locator type "${type}" is not directly supported in this plugin yet`,
          );
        }
      }
      ...
      this.logger.warn(
        `Failed to initialize Kubernetes clusters from config: ${error}. Will use default kubeconfig.`,
      );
    }
  }
  ...
  public getKubeConfig(
    ...
    if (clusterName) {
      ...
      if (!kubeConfig) {
        this.logger.info(
          `No configuration found for Kubernetes cluster "${clusterName}", falling back to default kubeconfig`,
        );
    ...
    this.logger.info(
      'No configured clusters available, falling back to local kubeconfig',
    );
    const fallbackKubeConfig = new k8s.KubeConfig();
    fallbackKubeConfig.loadFromDefault();
    return fallbackKubeConfig;
   ...
   */
  public getObjectsClient(
    options?: KubernetesClientFactoryOptions,
  ): k8s.KubernetesObjectApi {
    let kubeConfig = this.getKubeConfig(options);

    const currentUser = kubeConfig.getCurrentUser();
    if (currentUser?.authProvider === 'oidc' && options && options.token) {
      const currentContextName = kubeConfig.getCurrentContext();
      const context = kubeConfig.getContextObject(currentContextName);
