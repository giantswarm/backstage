...
export async function kubeApply(
  ...
  if (kubeClientFactory) {
    ...
    logger.info(
      `Using KubernetesClientFactory for cluster: ${clusterName || 'default'}`,
    );
  } else {
  ...
  for (const spec of validSpecs) {
    ...
    try {
      ...
      // block.
      logger.info(
        `Attempting to get resource ${spec.kind}/${spec.metadata.name}`,
      );
      await client.read(spec as any);
      logger.info(
        `Resource exists, attempting to patch ${spec.kind}/${spec.metadata.name}`,
      );
      ...
      logger.info(
        `Resource not found, attempting to create ${spec.kind}/${spec.metadata.name}`,
      );
