...
export function useCustomBoardNamespacedResources(cluster: string, customBoard: string, namespace?: string) {
    const { fetch } = useApi(fetchApiRef);

    return useAsync(async (): Promise<ResourceResultList | undefined> => {
        if (!customBoard) return undefined;

        const url = new URL(`plugin://policy-reporter/${cluster}/custom-board/${customBoard}/resource-results`);
        if (namespace) {
