...
import { useCustomBoardResourceDetails } from '../../hooks/useCustomBoardResourceDetails';
...
export const ResourcePage = () => {
    ...
    const { cluster, customBoard, resource } = useParams<{ cluster: string, customBoard: string, resource: string }>();
    
    const [searchParams] = useSearchParams();
    const source = searchParams.get('source') ?? undefined;
    const category = searchParams.get('category') ?? undefined;

    const { value: dashboard, loading, error } = useCustomBoardResourceDetails(cluster!, customBoard!, resource!);

    const pageTitle = dashboard ? `${dashboard?.resource.name}` : '';

    if (!cluster || !customBoard || !resource) {
        return (
