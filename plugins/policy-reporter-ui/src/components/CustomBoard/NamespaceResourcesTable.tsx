...
import { Container, Text, Table, ColumnConfig, useTable, Cell, CellText } from '@backstage/ui';
import { StatusComponent } from '../StatusComponent';

import type { PropsWithChildren } from 'react';
import { useCustomBoardNamespacedResources } from '../../hooks/useCustomBoardNamespacedResources';
import { ResourceResult } from '@internal/backstage-plugin-policy-reporter-common';

export const NamespaceResourcesTable = ({ cluster, namespace, customBoard }: PropsWithChildren<{ cluster: string; namespace: string; customBoard: string }>) => {
    const { value, loading, error } = useCustomBoardNamespacedResources(cluster, customBoard, namespace);
    ...
    const columns: ColumnConfig<ResourceResult>[] = [
        {
            ...
            width: '50%',
            isRowHeader: true,
            cell: (rowData: ResourceResult) => (<CellText title={rowData.name} href={appendQueryParams(`/policy-reporter-ui/clusters/${cluster}/custom-board/${customBoard}/resources/${rowData.id}`)} description={`${rowData.apiVersion} ${rowData.kind}`} />),
        },
