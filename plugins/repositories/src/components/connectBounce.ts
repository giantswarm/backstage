 ...
 * connect from the Repositories page, so a connect that comes back without a
 * grant is not repeated in a loop.
 */
export const CONNECT_BOUNCE_KEY = 'repositories.manager.connect-bounce';
 ...
 * state=…`) so the callback returns the browser to the page that needed the
 * manager. muster validates the target against its allowlist.
 */
