...
Kubernetes MCP tools are read-only.
...
Helm charts that are represented in the portal's catalog can be deployed easily through the portal's "App Deployment" page/flow at the URI `/create/templates/giantswarm/app-deployment`. The result is an OCIRepository and a HelmRelease resource in the management cluster. Applications deployed in this way can also be modified directly in the portal. There is an "Edit" button available on the deployment details page which allows the user to change the chart (OCI repository and tag), upgrade strategy (all, minor, patch, none), and the configuration (both confidential and non-confidential). Note: Editing only makes sense if the HelmRelease resource is not managed via gitops.
