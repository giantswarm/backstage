...
// eslint-disable-next-line @backstage/no-undeclared-imports -- openid-client v5 is hoisted from @backstage/plugin-auth-backend-module-oidc-provider
import {
  custom,
...
export const oidcAuthenticator = createOAuthAuthenticator({
  ...
  async authenticate(
    ...
    try {
      ...
      console.log(
        `[oidc-debug] authenticate succeeded, email=${result.userinfo.email}, groups=${Array.isArray(result.userinfo.groups) ? result.userinfo.groups.length : 'N/A'}, id_token_length=${result.tokenset.id_token?.length ?? 0}`,
      );
      ...
      console.error(
        `[oidc-debug] authenticate FAILED: name=${err.name}, message=${err.message}`,
      );
      if (err.response?.body) {
        console.error(
          `[oidc-debug] response body: ${JSON.stringify(err.response.body)}`,
        );
      }
      if (err.stack) {
        console.error(`[oidc-debug] stack: ${err.stack}`);
      }
      throw error;
    }
  },
