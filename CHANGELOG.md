...
- Cluster access without muster: `gs.clusterTokenBroker.targets.<installation>` (`tokenUrl`, `clientId`, `clientSecret`, `connectorId`, `scopes`) exchanges the main Dex session at that installation's own Dex (RFC 8693 with Dex's `connector_id`, an id_token for the apiserver's client), so a portal on a management cluster without the Agent Platform reaches a second installation through the one main login. The muster `tokenUrl` becomes optional; with both, the targets win for their installations. An installation with a target is broker-covered without `clusterTokenAudience`, and the browser learns only the target names (#2387).
- Chart: `observability.otel` (`endpoint`, `protocol`, `headers`, `resourceAttributes`, `sampler`, `samplerArg`) sets the backend's OTLP trace export as the standard OTEL_* variables, with the pod's `k8s.pod.name`, `k8s.namespace.name`, `k8s.node.name` and the chart's `service.version` as resource attributes. The sampler defaults to `parentbased_traceidratio` at `0.1`. Nothing renders while `endpoint` is empty. The image starts the OpenTelemetry SDK before the backend through `NODE_OPTIONS`, so a `backstage.command` override keeps tracing; an installation that sets its own `NODE_OPTIONS` keeps `--no-node-snapshot --require /app/instrumentation.js` in it (giantswarm/giantswarm#36711).
- MCP servers page: render muster's `Awaiting Session` state (a server used with each person's own identity through token forwarding or exchange, with no session connected) as healthy on the server rows, the family cluster pills, the dashboard stat and the detail view; the state badge and the cluster pills carry muster's `Ready` condition message as a tooltip and the Health block shows it as a row, so the page tells a per-session server that works from one whose token exchange is broken; the empty-tools note names the per-session mechanism instead of a Sign in, and the verify step treats the state as verified (#2529).
- Installations page: one column per platform capability with the state from giantswarm-platform-manager's `list_installations`, and a **Capabilities** tab per installation with the state, the inputs on record and the last action, Enable/Reconcile opening the manager's dry run with the inputs as a form generated from the definition's JSON schema (`get_info`), Commit disabled without the manager's grant and absent where the manager would refuse it (an installation not opted in shows the opt-in file's path and the pull request that adds it), Verify with the features and their marks, and the action history. New plugins `platform-capabilities` and `platform-capabilities-backend`, disabled by default (`app.extensions`: `api:platform-capabilities`, `entity-content:platform-capabilities/capabilities`; `platformCapabilities.muster`) (#2442).
- Chart: `database.postgresql.instances`, `database.postgresql.affinity` and `database.postgresql.imagePullSecrets` reach the CNPG `Cluster`. `affinity` is CloudNativePG's own `AffinityConfiguration`, not a core Kubernetes `Affinity`. Unset, all three render as before (#2342).
- Chart: `networkPolicy.enabled` and `networkPolicy.flavor` (`cilium` or `kubernetes`). The CNPG network policy was previously a `CiliumNetworkPolicy` that always rendered, so `database.engine: postgresql` could not install on a cluster without the Cilium CRDs. The `kubernetes` flavor is not an exact equivalent: it has no `world` or `kube-apiserver` entity, so the egress leg that carries object storage and Kubernetes API traffic is `0.0.0.0/0` on 443 and 6443 (#2340, #2341).
- Chart: `database.postgresql.operatorNamespace`, the namespace the CloudNativePG operator runs in, defaulting to `cnpg-system` (#2341).
- Catalog: show release readiness in the component catalog. A sortable "Release readiness" column, a sidebar filter, and a per-component card that names each blocker and what to do about it. Blocked sorts first, and `unknown` renders distinctly from both `releasable` and `blocked` rather than as a blank — "we could not tell" and "nothing wrong" are different answers, so a verdict with no blockers to name explains itself on hover instead. The column appears where `AppReadinessProcessor` has run, since a release-readiness cell needs its verdict; the card appears wherever there is readiness data from either source, because most of what it shows comes from the catalog importer rather than the processor. The card keeps three claims apart: the release verdict, the importer's chart-metadata verdict (`giantswarm.io/readiness-standards`), and advisory gaps. Both verdicts write to `giantswarm.io/readiness-flags`, so the card splits that list back apart by the flag that owns it — "Blocking the release" versus "Fails a build today" — rather than presenting a chart-metadata gap as something stopping a release. Advisory gaps are labelled "not enforced" and shown neutrally, because four charts in five carry at least one and none of them blocks anything.
- Catalog: a new `AppReadinessProcessor` records whether a component's newest GitHub release actually reached the chart registry, as the `giantswarm.io/readiness` label (`releasable` / `blocked` / `unknown`) plus `giantswarm.io/readiness-flags` and `giantswarm.io/readiness-checked`. This is the release precondition for the HelmRelease migration: a chart that was never published has nothing for a HelmRelease to point at, however healthy the repo looks. `unknown` is a first-class verdict — a chart that cannot be resolved, a private registry we hold no credentials for, a monorepo release prefix, or a release tag that is not comparable semver all yield `unknown`, never `blocked`. Flags merge with whatever the catalog importer already published under the same key, so the release verdict and the chart-metadata verdict share one list. Off by default; enable with `catalog.processors.appReadiness.enabled`.
...
- The GPU node pool's _Remove anyway_ says what force does now: the nodes go with the pool, the workloads on them are evicted, and with the cluster's last pool the models the platform serves are removed with it (cluster-manager removes them with the controller's finalizer taken off, giantswarm/cluster-manager#90); before, a model left behind read _Stopping_ for good once stopped.
- Capabilities tab: one block per capability, one comparison and one action. The comparison with the definition runs as the tab opens and the header line says what it found — `Installed · up to date`, `Installed · 3 differences · 2 planned changes`, `Installed · not compared`, `Not installed`, `Enabling · pending approval`, `Applying · rolling out`, `Waiting for the customer: <action>`, `Failed: <probe>`; under it one line per choice a person makes (`Model serving: off`), each feature with differences opening to them, the features whose changes a migration plans and the features as defined on one line each, the checks that did not run with their reason, and one button, Enable or Apply changes, whose dialog reviews the same comparison with the form's values before it opens the pull requests. An installation whose owners have not allowed the manager has the button disabled and one line naming the file to add. Gone: the Verify button, the definition's description, the inputs on record, the last-action line and the manager's state words. The Installations page icons keep their shape and use the tab's words in their tooltips. Needs giantswarm-platform-manager 0.27.3 or later, whose `verify_capability` takes `inputs`, answers with the plan and marks a migration's changes `planned` (#2509).
- Models → Serving: each backend's table opens with the running models (Ready), then the ones that need attention (Not serving, Not ready, Pending, Stopping), then the ones not running (Idle, Downloading, Available), the name as the tiebreaker. It opened sorted by name, and a sort by Status followed the alphabet of the state words, so a broken model could sit between two available ones (#2496).
- Usage → Cost: the by-agent table says what a row is. An agent the portal knows is named and linked; a pair of gateway labels that matches no agent is a removed agent, shown by its technical name and marked **Removed**, unlinked, its spend kept in the totals; the gateway's own `unknown` reads as Unattributed. The note under the table describes attribution as it is under kagent API v2: the runtime names the agent and the person on each model call, the gateway records them for a call arriving through the Substrate egress and attributes any other call to the calling pod's ServiceAccount (#2489).
- Repositories: Align now on a repository that has not opted in to alignment opts it in and aligns it. The dialog renders the manager's three modes: opted in (`align: true` in its entry), the set-up workflow is dispatched and the button reads Align now; declared but not opted in, the dry run shows the entry as it will read, the pull request and the ask the team's channel receives, the button reads Opt in and align, the commit opens that pull request as the person and the reconciler aligns the repository when it merges -- nothing is dispatched; without an entry, the run checks from the team alone and the button reads Check now. The opt-in line names the repository, not its team. `Alignment.mode` gains `opt-in` and `Alignment.optIn` carries the plan and, after the commit, the pull request and the delivered ask (#2472).
- Bot PRs: the page reads every team of a scope in one `x_marge_list` call, with the new `teams` argument, instead of one call per team. marge merges the teams' repository lists and lists them once, and still decides every team under its own team file. The team filter of the bar now narrows what is shown and reads nothing, because the team is already in the queue; a `?team=` the scope does not hold is still read on its own. Needs a marge that takes `teams`: an older one answers the query scope, which the page refuses rather than renders.
- Chart: the default memory limit is `1Gi` (was `600Mi`) with a `512Mi` request (was `250Mi`). Node sizes the V8 heap from the container's memory limit (348 MiB under 600Mi, 560 MiB under 1Gi) and the backend's native memory adds about 250-300 MiB on top, so under the portal's browser e2e suite the 2.8.1 image's main thread was OOM-killed at 600Mi; at 1Gi it peaks at about 660 MiB with 0 restarts. The measurement is in the chart README ("Memory"). Installations that override `resources` are unaffected (#2396).
...
- Installations page: the **Consistency** views (`/installations/consistency/<capability>`, one tab per platform capability comparing every installation of the registry against the definition's features). The comparison joins the installations overview over time; until then the capability columns and an installation's Capabilities tab (**Verify**) are the two places (#2443).
...
See [./docs/releases/v0.130.0-changelog.md](./docs/releases/v0.130.0-changelog.md) for more information.
...
See [./docs/releases/v0.129.0-changelog.md](./docs/releases/v0.129.0-changelog.md) for more information.
...
See [./docs/releases/v0.128.1-changelog.md](./docs/releases/v0.128.1-changelog.md) for more information.
...
See [./docs/releases/v0.110.2-changelog.md](./docs/releases/v0.110.2-changelog.md) for more information.
...
See [./docs/releases/v0.103.0-changelog.md](./docs/releases/v0.103.0-changelog.md) for more information.
...
- Add OCIRepository support for HelmRelease source resolution and chart name lookup.
...
[Unreleased]: https://github.com/giantswarm/backstage/compare/v0.138.0...HEAD
[0.138.0]: https://github.com/giantswarm/backstage/compare/v0.137.2...v0.138.0
[0.137.2]: https://github.com/giantswarm/backstage/compare/v0.137.1...v0.137.2
...
[0.96.0]: https://github.com/giantswarm/backstage/compare/v0.95.1...v0.96.0
