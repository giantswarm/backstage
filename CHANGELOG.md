...
- Chart: `observability.otel` (`endpoint`, `protocol`, `headers`, `resourceAttributes`, `sampler`, `samplerArg`) sets the backend's OTLP trace export as the standard OTEL_* variables, with the pod's `k8s.pod.name`, `k8s.namespace.name`, `k8s.node.name` and the chart's `service.version` as resource attributes. The sampler defaults to `parentbased_traceidratio` at `0.1`. Nothing renders while `endpoint` is empty. `backstage.command` defaults to `node --require ./instrumentation.js packages/backend`, which starts the OpenTelemetry SDK before the backend; an installation that overrides `command` keeps `--require ./instrumentation.js` in it to export traces (giantswarm/giantswarm#36711).
