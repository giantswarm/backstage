...
- Catalog: show release readiness in the component catalog. A sortable "Release readiness" column, a sidebar filter, and a per-component card that names each blocker and what to do about it. The column and the card only appear where `AppReadinessProcessor` has run, so instances without it see no empty shell. Blocked sorts first, and `unknown` renders distinctly from both `releasable` and `blocked` rather than as a blank — "we could not tell" and "nothing wrong" are different answers. The card keeps enforced and advisory gaps visually apart: enforced gaps fail a build today and read as problems, while advisory gaps are labelled "not enforced" and shown neutrally, because four charts in five carry at least one and none of them blocks anything.
