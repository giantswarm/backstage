...
- Installations page: a **Consistency** view per platform capability, as a tab beside the list: one row per installation of the registry (opted in or not, the hub among them), one column per feature of the capability's definition (`get_info`), each cell the feature's mark from `verify_capability` — _as defined_, _differs by input_, _drifted_, or _not checked_ with the manager's reason. A cell expands to its dimensions with the differences (file, path, the input driving it, rendered against current) and the probes' answers; a row to the installation's inputs on record. The comparison runs when the view opens, four installations at a time, is kept for the session, and **Verify now** runs it again for one installation; there is no schedule. An installation the person may not read — the portal's inventory probe (`GET /apis` through the kubernetes proxy) refused with 401 or 403 — shows its live dimensions (probes, live comparison) as _not readable_, not as drift, and the feature's mark rolls up from the file dimensions alone (#2443).
...
- Models → Serving: each backend's table opens with the models that answer requests (Ready, then Idle), then the ones that need attention (Not serving, Not ready, Pending, Stopping), then the inventory nothing runs yet (Downloading, Available), the name as the tiebreaker. It opened sorted by name, and a sort by Status followed the alphabet of the state words, so a broken model could sit between two available ones (#2496).
