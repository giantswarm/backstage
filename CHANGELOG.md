...
- The GPU node pool's *Remove anyway* says what force does now: the nodes go with the pool, the workloads on them are evicted, and with the cluster's last pool the models the platform serves are removed with it (cluster-manager removes them with the controller's finalizer taken off, giantswarm/cluster-manager#90); before, a model left behind read *Stopping* for good once stopped.
