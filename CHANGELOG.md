...
- Capabilities tab: one block per capability, one comparison and one action. The comparison with the definition runs as the tab opens and the header line says what it found — `Installed · up to date`, `Installed · 3 differences`, `Not installed`, `Enabling · pending approval`, `Applying · rolling out`, `Waiting for the customer: <action>`, `Failed: <probe>`; under it one line per choice a person makes (`Model serving: off`), each feature with differences opening to them, the features as defined on one line, the checks that did not run with their reason, and one button, Enable or Apply changes, whose dialog reviews the same comparison with the form's values before it opens the pull requests. An installation whose owners have not allowed the manager has the button disabled and one line naming the file to add. Gone: the Verify button, the definition's description, the inputs on record, the last-action line and the manager's state words. The Installations page icons keep their shape and use the tab's words in their tooltips. Needs giantswarm-platform-manager 0.26.0 or later, whose `verify_capability` takes `inputs` and answers with the plan (#PRNUMBER).
