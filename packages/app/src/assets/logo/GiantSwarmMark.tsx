...
import { ANT_VIEW_BOX, GiantSwarmAntPaths } from './GiantSwarmAnt';

/**
 * The Giant Swarm ant on its own, in brand colors. Used where the horizontal
 * lockup does not fit — the collapsed sidebar rail — and cropped from the same
 * artwork as the lockup so the two match when the sidebar expands.
 *
 * For a single-color ant that inherits the surrounding text color, use
 * `GiantSwarmIconPaths` instead: that is the UI glyph (auth provider lists, the
 * app's icon bundle), not the logo.
 */
export const GiantSwarmMark = (props: { className?: string }) => {
  const gradientId = `gs-mark-ant-${useId().replace(/:/g, '')}`;

  return (
    <svg
      {...props}
      xmlns="http://www.w3.org/2000/svg"
      viewBox={ANT_VIEW_BOX}
      role="img"
...
/**
 * The single-color ant, as bare paths for embedding in a `SvgIcon` (viewBox
 * `0 0 354.3 354.3`). Renders in `currentColor`: this is the UI glyph, so it
 * follows the surrounding text rather than carrying the brand gradient.
 */
export const GiantSwarmIconPaths = () => (
  <>
    <path
      fill="currentColor"
