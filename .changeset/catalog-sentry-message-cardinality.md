...
round retries is now `debug`; anything else keeps a fixed message with the
chart and error as structured metadata.
...
faults, timeouts) drop to `debug`. Everything else stays at `warn` with the
