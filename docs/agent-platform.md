...
Each page carries a "Step X of 3" label. Only step 1 validates (name, slug,
...
- **Filters within that grouping.** The search field matches name, description,
  ...
  The `AccordionGroup` is keyed on the matching repo set because bui's
  `defaultExpandedKeys` only applies on mount — without that, repos that start
  matching mid-search would stay collapsed.
