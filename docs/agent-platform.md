...
The **marge** tab (`/agent-platform/marge/<team>`) shows one team's open
bot PRs (Renovate, Dependabot, Align files, Herald) as
[marge](https://github.com/giantswarm/marge), the sweep engine, classifies
them, and fires the sweep's own steps on them. The portal composes nothing:
every read and every write is one of marge's four MCP tools, called through
the installation's muster as the signed-in person (`MargeClient`, the same
seam as agent-manager). There is no agent in the path.

| Control                                  | Tool call                                                                                                               |
| ---------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| The table                                | `x_marge_list` with `team`: the stored classification, read from each PR's `marge/<class>` label in one search          |
| **Refresh classification**               | `x_marge_list` with `refresh: true`: the engine classifies every PR again, one check read per PR                        |
| **Preview sweep**                        | `x_marge_sweep` with `dry_run: true`                                                                                    |
| **Apply sweep**                          | `x_marge_sweep` with `prs` set to exactly the PRs the preview listed                                                    |
| **Merge…** / **Refresh branch…** on a PR | `x_marge_sweep` with `prs: [<the PR>]` and `actions` narrowed to `approve,merge,mark` / `refresh,mark`, previewed first |
| **Remedy…** on a PR                      | `x_marge_remedy`: the catalogue rule that matches the PR, under its own guards, previewed first                         |
| **Mark blocked…** on a PR                | `x_marge_mark` with `outcome: blocked`, `tool: developer-portal` and the person's reason                                |

### What the table is, and is not

The table is the **stored** read. marge keeps one classification label on
every PR a sweep touched, and `x_marge_list` reads it back in one search: what
the last sweep decided, not what a sweep would decide now. A PR no sweep has
labelled comes back `Unclassified` and is shown as such, with the count in the
summary line, rather than hidden or guessed at. The stored read carries no
update type, no policy and no rescue state; those columns fill on a live read.

The **live** read (`refresh: true`) is a click, and only a click: nothing on
the page classifies on mount, on focus or on a timer. Its answer replaces the
stored one in the same cache entry and stays until the next read. The summary
line says which of the two the table shows and when it was read.

The queue is one person's view -- marge acts with their GitHub grant -- so the
cache entry lives under the `muster` query-key prefix: never persisted, and
invalidated by the muster plugin's sign-in flow, which is what makes the queue
load by itself once the person connects.

### Preview and Apply

Preview is the sweep with `dry_run: true`: a live classification of every PR
in scope and the step the engine would take on each, shown per PR with the
engine's state and evidence line. Apply repeats the call without `dry_run` and
with `prs` set to the PRs the preview listed, so a PR that appeared in between
is not swept unseen. Under a team policy with `rescue.confirm: per-pr` (the
company default, and what a stored read without a policy reads as) the
preview's rows carry checkboxes and Apply runs on the ticked ones; under
`per-sweep` one confirmation covers the run. The confirm mode is read off the
`policy` the entries carry.

A guard that refuses shows up as an outcome row -- `Skipped`, `Held`, a
`Failed` with the guard's reason in the detail -- and nothing offers a way
around it, because the engine has none. A refusal of the whole call (a team
without a team file, a PR outside the scope) is the tool's error, shown
verbatim.

### Why there is no rerun and no rescue button

`rerun` is not a tool and not a sweep step. It is a remedy action a catalogue
rule can name, and whether a failure wants a rerun is that rule's decision
under its guards (roadmap#4355). The per-PR **Remedy…** is therefore
`x_marge_remedy` on that PR: the preview names the rule that would apply, or
reports the failure under `unhandled` when none does. A direct rerun button
would route around the guards.

`rescue` has no tool yet; the engine step is roadmap#4360. The rescue column
shows the prior attempt marge found on the PR, and **Mark blocked…** records
that a person looked; dispatching one is not offered.

### Team and installation

The team is a path segment, so one team's queue is one link. The index
redirects to the person's own team, read from their identity's ownership
refs; the selector lists the catalogue's `Group` entities of type `team`,
and a free field takes any other name. Giant Swarm's groups are `team-<name>`
and marge's team files `team-<name>.yaml`, so the prefix is dropped on the
way to the tool; whether a team file exists is marge's answer.

The page reaches marge through one muster: the section's pinned installation
when its muster lists a `marge` MCPServer, else the home installation, else
the first in scope that does (`useMargeInstallation`, the same resolution as
the Usage tab). A pinned installation without marge resolves to nothing and
says why.

### Sign-in

A person without a GitHub grant for marge gets the muster plugin's existing
per-server `ServerSignIn`, wrapped in an alert that says what the grant is for.
The flow is muster's: a popup to GitHub that closes itself, `auth://status`
flipping to connected, the muster plugin invalidating its reads. The page
adds nothing to it, and re-reads the queue when the status flips.

Every write lands as the person: the evidence comment, the marker and the
merge carry their GitHub login, not the sweep App's. That is marge's contract
(giantswarm/marge#124) and is what the page relies on; nothing here attributes
anything itself.
