...
|------------|------|---------|
| oci://registry-1.docker.io/bitnamicharts | common | 2.39.0 |
...
| replicas | int | `1` | Number of Backstage pod replicas. Defaults to 1, which is the only safe value when the deployment is backed by a ReadWriteOnce PVC (file-backed SQLite). Multi-replica deployments require either an external Postgres database (set `database.engine: postgresql`) or a ReadWriteMany volume. |
| readinessProbe | object | `{}` | Override the readiness probe applied to the Backstage container. Fields specified here override the chart defaults (`/.backstage/health/v1/readiness`, `initialDelaySeconds: 5`, `periodSeconds: 10`). Useful for slow cold-starts where the default 5s delay races plugin initialisation. |
| livenessProbe | object | `{}` | Override the liveness probe applied to the Backstage container. Fields specified here override the chart defaults (`/.backstage/health/v1/liveness`, `initialDelaySeconds: 10`, `periodSeconds: 15`). Bump `initialDelaySeconds` and `failureThreshold` for plugin-heavy installs whose cold-start exceeds 30-60s and would otherwise be killed mid-init. |
